...
// Obtener el dbnos de la URL o usar uno por defecto
function obtenerDbnosDeURL() {
  const params = new URLSearchParams(window.location.search);
  return params.get('dbnos') || "47"; // Valor por defecto si no se especifica
}

// Contenido específico de la partitura (no cambia entre cantos)
const contenidoPartitura = {
  cantor: [
    ...
    /* 26 */ "EL SEÑOR REDIME A ISRAEL BIS",
    /* 27 */ "DE TODOS SUS PECADOS.",
    /* 28 */ "¡Ay!",
  ],
  cantorAcordes: [
    /* 1*/  [ac("La", "cp14","m"), ac("Fa","cp90","7"), ac("Mi","cp150",""), ac("La","cp250","m")],
...
// Cargar datos del canto desde IndexCantos.js
async function cargarDatosCanto(dbnos) {
  try {
    // Importar dinámicamente el módulo
    const { getCantoByDbnos } = await import('../../src/js/IndexCantos.js');
    const canto = getCantoByDbnos(dbnos);
    
    if (!canto) {
      console.error(`Canto con dbnos ${dbnos} no encontrado`);
      return null;
    }
    
    return {
      tituloc: canto.nombredelcanto,
      titulo: canto.nombredelcanto,
      salmo: canto.salmo,
      dbnos: canto.dbnos,
      catg: canto.catg,
    };
  } catch (error) {
    console.error("Error al cargar los datos del canto:", error);
    return null;
  }
}
...
// Variable global para la partitura
let partitura;

document.addEventListener('DOMContentLoaded', async () => {
  const dbnos = obtenerDbnosDeURL();
  partitura = await cargarDatosCanto(dbnos);
  
  if (!partitura) {
    // Manejar el caso de error mostrando un mensaje al usuario
    const contenedor = document.getElementById('contenedor-partitura');
    if (contenedor) contenedor.innerHTML = '<p>No se pudo cargar el canto solicitado</p>';
    return;
  }
