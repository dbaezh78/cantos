...
function crearSelectorTonica(tonicaActual) {
    return `
        <div class="selector-tonica">
            <label>Transportar a: </label>
            <select id="tonica-principal" class="no-arrow">
                ${acordesCromaticos.map(a =>
                    `<option value="${a}" ${a === tonicaActual ? 'selected' : ''}>${a}</option>`
                ).join('')}
            </select>
            <button id="reset-transpose">Original</button>
        </div>
    `;
}
...
function crearVersoHTML(verso, index) {
    return `
        <div class="line" data-verso-id="${index}">
            ...
            <div class="lyrics">${verso.texto || ''}</div>
        </div>
    `;
}
...
function calcularAcordeDesplazado(acordeCompleto, desplazamiento) {
  ...
  let indiceNuevo = (indiceOriginal + desplazamiento + acordesCromaticos.length) % acordesCromaticos.length;
  let baseNueva = acordesCromaticos[indiceNuevo];

  // Determinar la "calidad" del intervalo (diatónico vs. cromático)
  const esCromaticoOriginal = baseOriginal.includes('#') || baseOriginal.includes('♭');
  const esCromaticoNuevo = baseNueva.includes('#') || baseNueva.includes('♭');

  // Ajustar la nota nueva basándonos en la calidad del intervalo y la dirección
  if (desplazamiento > 0) { // Desplazamiento hacia arriba
      if (esCromaticoOriginal && !esCromaticoNuevo && acordesCromaticos[(indiceOriginal + 1) % acordesCromaticos.length] !== baseNueva) {
          baseNueva = acordesCromaticos[(indiceNuevo + 1) % acordesCromaticos.length]; // Intentar con el siguiente
          if (!baseNueva.includes('#') && !baseNueva.includes('♭')) {
              baseNueva += '#'; // Añadir sostenido si es necesario
          }
      } else if (!esCromaticoOriginal && esCromaticoNuevo && acordesCromaticos[indiceNuevo] !== baseOriginal) {
          baseNueva = baseNueva.replace('#', '').replace('♭', ''); // Quitar alteración si es posible
      }
  } else if (desplazamiento < 0) { // Desplazamiento hacia abajo
      if (!esCromaticoOriginal && esCromaticoNuevo && acordesCromaticos[(indiceOriginal - 1 + acordesCromaticos.length) % acordesCromaticos.length] !== baseNueva) {
          baseNueva = acordesCromaticos[(indiceNuevo - 1 + acordesCromaticos.length) % acordesCromaticos.length]; // Intentar con el anterior
          if (!baseNueva.includes('#') && !baseNueva.includes('♭')) {
              baseNueva += '♭'; // Añadir bemol si es necesario (esto podría necesitar más ajustes)
          }
      } else if (esCromaticoOriginal && !esCromaticoNuevo && acordesCromaticos[indiceNuevo] !== baseOriginal) {
          baseNueva = baseNueva.replace('#', '').replace('♭', ''); // Quitar alteración si es posible
      }
